...
public class MethodAbstractionTool {
    ...
    public static void main55(String[] args) {
        String methodCode1 = "public void updateCheckerLikeRefByPush() throws Exception {\n" +
                "    String checkerRef = CheckerUuid.parse(\"foo:bar\").toRefName();\n" +
                "\n" +
                "    allow(checkerRef, Permission.CREATE, adminGroupUuid());\n" +
                "    createBranch(new Branch.NameKey(project, checkerRef));\n" +
                ...
                "    r.assertOkStatus();\n" +
                "  }";
        String methodCode2 = "private static boolean onSupportedJavaVersion() {\n" +
                "    final String version = System.getProperty(\"java.specification.version\");\n" +
                "    if (1.7 <= parse(version)) {\n" +
                "      return true;\n" +
                "\n" +
                ...
                "    return false;\n" +
                "  }";
        String methodCode3 = "private static double parse(String version) {\n" +
                "    if (version == null || version.length() == 0) {\n" +
                "      return 0.0;\n" +
                "    }\n" +
                "\n" +
                ...
                "    }\n" +
                "  }";
        String methodCode = "public ImmutableList<CheckInfo> list(ListChecksOption... options) throws RestApiException {\n" +
                "    try {\n" +
        ...
        MethodAbstractionTool tool = new MethodAbstractionTool();
        String res = tool.abstractMethod(methodCode);
